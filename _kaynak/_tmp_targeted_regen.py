...
TARGET_SLUGS = {
    "500-tavukla-yumurta-isine-baslamak",
    "gezen-tavuk-sistemi-kurmak",
    "tavuk-basina-kac-m2-gerekir",
    "koye-donus-tavukculuk-hatalari",
}
arts = load_articles()
arts = [a for a in arts if a.get("order", 0) <= 20]  # sadece committed 01-20 seti; taslak 21 haric
by_slug = {r["slug"]: r for r in arts}
for rec in arts:
    if rec["slug"] not in TARGET_SLUGS:
        continue
    d = os.path.join(PROJ, "blog", rec["slug"])
    out_path = os.path.join(d, "index.html")
    new_html = article_page(rec, by_slug)
    open(out_path, "w", encoding="utf-8").write(new_html)
    print("regenerated:", rec["slug"])
