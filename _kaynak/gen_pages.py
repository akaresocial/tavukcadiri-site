...
#cookie{position:fixed;left:14px;right:14px;bottom:14px;z-index:120;background:#243A1E;color:#EAF0E8;border-radius:16px;padding:16px 18px;display:none;gap:14px;align-items:center;flex-wrap:wrap;box-shadow:0 20px 50px -20px rgba(0,0,0,.5)}
#cookie p{margin:0;font-size:14px;line-height:1.5;flex:1 1 280px}
#cookie a{color:#F3B775;text-decoration:underline}
#cookie button{background:#1FA855;color:#fff;border:0;border-radius:10px;padding:11px 20px;font-weight:600;font-size:14.5px;cursor:pointer}
...
def header(pre):
    L=[("Modeller",pre+"index.html#modeller"),("Fiyatlar",pre+"fiyatlar/"),("Özellikler",pre+"index.html#ozellikler"),("Neden Biz",pre+"index.html#referanslar"),("S.S.S.",pre+"index.html#sss"),("İletişim",pre+"iletisim/")]
...
def footer(pre):
    ...
    return ('<footer><div class="wrap"><div class="foot-grid">'
      ...
      '<div><h5>Kurumsal</h5><ul><li><a href="%shakkimizda/">Hakkımızda</a></li><li><a href="%siletisim/">İletişim</a></li><li><a href="%sindex.html#sss">S.S.S.</a></li></ul></div>'
      '<div><h5>Yasal</h5><ul><li><a href="%skvkk/">KVKK Aydınlatma Metni</a></li><li><a href="%sgizlilik/">Gizlilik &amp; Çerez Politikası</a></li></ul></div>'
      '</div><div class="foot-bottom"><span>© 2026 Tavuk Çadırı. Tüm hakları saklıdır.</span><span>Üretim &amp; kurulum: DEHA Çadır · 81 il</span></div></div></footer>')%(pre,WA,WA_SVG.format(w=17,f="#fff"),m,pre,pre,pre,pre,pre)

COOKIE=('<div id="cookie"><p>Bu sitede, deneyiminizi iyileştirmek ve temel işlevler için çerezler kullanıyoruz. Detaylar için <a href="{pre}gizlilik/">Gizlilik &amp; Çerez Politikası</a>.</p><button onclick="cookieOk()">Tamam</button></div>')
...
def doc(title,desc,slug,body_main,pre="../",noindex=False,extra_js=""):
    ...
    return ('<!DOCTYPE html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">'
      ...
      e(title),e(desc),robots,canon,e(title),e(desc),og,pre,pre,pre,CSS,header(pre),body_main,footer(pre),COOKIE.format(pre=pre),JS,('<script>%s</script>'%extra_js if extra_js else ''))
...
about_body=(phead("Hakkımızda","Anahtar teslim tavuk çadırında güvenilir çözüm ortağınız",
 ...
 +'<p>Tavuk Çadırı olarak amacımız, tavuk ve kümes yetiştiriciliğine başlamak ya da işini büyütmek isteyen üreticilere; betonarmenin yüksek maliyeti ve uzun izin süreçleri olmadan, hızlı kurulan, izolasyonlu ve dayanıklı bir barınak çözümü sunmaktır. 500 tavuktan 10.000 ve üzerine kadar her kapasitede, ölçüye özel üretim yapıyoruz.</p>'
...
contact_body=(phead("İletişim","Teklif ve bilgi için bize ulaşın",
 ...
 +'<a href="tel:+%s"><span class="ic">%s</span>Telefon</a>'%(WA,tel_svg)
 +'</div>'
...
kvkk_body=(phead("KVKK Aydınlatma Metni","Kişisel Verilerin Korunması Aydınlatma Metni",
 ...
 +'<section class="sec"><div class="wrap"><div class="prose">'
 +'<div class="note"><strong>Not:</strong> Şirket bilgileri güncellenmiştir. Yasal metinlerin nihai halini yayına almadan önce bir hukuk danışmanına kontrol ettirmeniz önerilir.</div>'
...
giz_body=(phead("Gizlilik & Çerez Politikası","Gizlilik ve Çerez Politikası",
 ...
 +'<section class="sec"><div class="wrap"><div class="prose">'
 +'<div class="note"><strong>Not:</strong> Nihai hali için yayın öncesi bir hukuk danışmanına kontrol ettirmeniz önerilir.</div>'
